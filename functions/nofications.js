...
const workers = {
    helloWorld: (notificationToken) => getTheParks(5, notificationToken, location)
}

const getTheParks = async (radius, token, location) => {
...
exports.taskRunner = functions.runWith({ memory: '2GB' }).pubsub
    ...
    .schedule('* * * * *').onRun(async context => {
        ...
        tasks.forEach(snapshot => {
            const { worker, notificationToken, location } = snapshot.data();
